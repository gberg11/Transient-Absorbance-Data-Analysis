...
class DataPlotterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Transient Absorbance Data Visualization")
            ...
            try:
                ...
                for i, color in zip(range(0, len(self.data.columns), 2), colors):
                    ...
                    if self.use_filter.get():
                        ...
                        ax_line, = self.ax.plot(self.data[wavelength_col], smoothed_data, label=f'{delay_time}', color=color, linewidth=1.5, clip_on=True)
                        if self.plot_raw_data.get():
                            self.ax.plot(self.data[wavelength_col], self.data[data_col], 'o', markersize=2, color=color, alpha=0.5)
                    else:
                        ax_line, = self.ax.plot(self.data[wavelength_col], self.data[data_col], label=f'{delay_time}', color=color, linewidth=1.5, clip_on=True)
        ...
        if self.canvas:
            ...
            if file_path:
                if file_path.endswith(".png"):
                    ...
                    self.canvas.figure.savefig(file_path, dpi=300, format="pdf")
                messagebox.showinfo("Save Plot", f"Plot saved as {file_path}")
        else:
            messagebox.showwarning("Warning", "No plot to save. Please plot the data first.")
